...
DDL_QUERY="CREATE SCHEMA IF NOT EXISTS bdp; DROP TABLE bdp.aircrafts_csv_table; CREATE EXTERNAL TABLE IF NOT EXISTS bdp.aircrafts_csv_table (distance_moved double, fuel_consumed float, flight_id string) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' STORED AS TEXTFILE LOCATION 'hdfs://namenode:8020/aircrafts_csv_table';"
DML_QUERY="LOAD DATA INPATH '/aircraft_data_processed/part-*' OVERWRITE INTO TABLE bdp.aircrafts_csv_table;"
